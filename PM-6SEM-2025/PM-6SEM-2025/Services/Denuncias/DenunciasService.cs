...
namespace PM_6SEM_2025.Services.Denuncias
{
    public class DenunciasService : IDenunciasService
    {
        ...
        {
            var resposta = new ResponseModel<List<DenunciasModel>>();
            ...
            try
            {
                var denuncia = new DenunciasModel
                {
                    ...
                    // ✅ guarda direto o base64 na coluna ArquivoFoto (string)
                    ArquivoFoto = string.IsNullOrWhiteSpace(denunciaDTO.ArquivoFotoBase64)
                        ? null
                        : denunciaDTO.ArquivoFotoBase64.Trim()
                };

                _context.Denuncias.Add(denuncia);
                await _context.SaveChangesAsync();

                var lista = await _context.Denuncias
                    .Include(d => d.Usuario)
                    .Include(d => d.ProblemaAcessibilidade)
            ...
            catch (Exception ex)
            {
                ...
                resposta.Status = false;
                resposta.Mensagem = inner != null
                    ? $"Erro ao criar denúncia: {ex.Message} | Detalhe: {inner}"
                    : $"Erro ao criar denúncia: {ex.Message}";
